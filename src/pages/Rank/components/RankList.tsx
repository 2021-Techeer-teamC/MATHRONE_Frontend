...
const Medal = (rank: data) => {
  // 순위에 따라서 메달 이미지 변경
  if (rank.rank - 1 === 1) return <CardMedia component="img" sx={{ width: 30 }} image={Gold} alt="gold medal" />;
  else if (rank.rank - 1 < 5) {
    return <CardMedia component="img" sx={{ width: 30 }} image={Silver} alt="silver medal" />;
  } else if (rank.rank - 1 < 11) {
    return <CardMedia component="img" sx={{ width: 30 }} image={Bronze} alt="bronze medal" />;
  } else {
    return <Typography />;
  }
};
