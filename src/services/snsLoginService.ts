...
class snsLoginService {
  ...
  signInWithGoogle(code: null | string) {
    return axios.post<signInUserItem>(`${process.env.REACT_APP_IP}/user/oauth/callback/google`, { code: code });
  }

  signInWithKakao(code: null | string) {
    return axios.post<signInUserItem>(`${process.env.REACT_APP_IP}/user/oauth/callback/kakao`, { code: code });
  }
  ...
  signOutWithGoogle() {
    return axios.post(`${process.env.REACT_APP_IP}/user/google/logout`, {
      headers: {
        Authorization: localStorage.getItem('accessToken'),
      },
    });
